...
%                             'StartPoint', [0.013, kappa/2, kappa/2, Q_fit.x0+peakdirection*sqrt(g2estimate), g2estimate]); % split fit, {'A','kin','LS','x0','g2'}
            ...
            Q_obj.modeFitQobj=modeFitQobj(indexorder);
        ...
        function [trans_raw, trans_fit, mzi_raw] = get_Q_rawdata(Q_obj,n)
            if n<=length(Q_obj.modeQ0)
                peakstart=Q_obj.modeStart(n);
                peakend=Q_obj.modeEnd(n);
                peakpos=Q_obj.modePos(n);
                ...
                Q_fit_obj = Q_obj.modeFitQobj{n};
                
                peakstart = max(peakstart - 200*(peakpos-peakstart),1);
                peakend = min(peakend + 200*(peakend-peakpos),length(Q_obj.trace_Q) );
                % plot_base=peakstart-peakpos:peakend-peakpos;
                % % xlim([min(plot_base) max(plot_base)]/MZI_fit_T*Q_obj.MZI_FSR);
                % scatter(plot_base/MZI_fit_T*Q_obj.MZI_FSR,(Q_obj.trace_MZI(peakstart:peakend)-MZI_fit_A0)/MZI_fit_A*0.2,'ko');
                % plot(plot_base/MZI_fit_T*Q_obj.MZI_FSR,(MZI_fit-MZI_fit_A0)/MZI_fit_A*0.2,'c','LineWidth',2);
                ...
                % plot(plot_base/MZI_fit_T*Q_obj.MZI_FSR,Q_fit/Q_fit_A,'r','LineWidth',2);
                % legend('MZI interferogram','Sinusoidal fit','Transmission','Lorentzian fit','Location','best');
                trans_raw = Q_obj.trace_Q(peakstart:peakend) /Q_fit_A;
                trans_fit = Q_fit_obj(peakstart-peakpos:peakend-peakpos) /Q_fit_A;
                
                mzi_raw = (Q_obj.trace_MZI(peakstart:peakend)-MZI_fit_A0);
%                 mzi_fit = (MZI_fit-MZI_fit_A0);
                % Q0=Q_obj.modeQ0(n);
                % Q1=Q_obj.modeQ1(n);
                % QL=Q_obj.modeQL(n);
                % transmission=Q_obj.modeTransmission(n);
