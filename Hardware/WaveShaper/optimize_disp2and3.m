...
filedir_glob = 'Z:\Maodong\Projects\Pulse Pumping\AlGaAs\AutoCorrelators\20211007-autocorr-Find-disp2and3'

center = 193.37;
bandpass = [0.8 0.75 0.7 0.6 0.5 0.4] ; %THz
disp2_list = -5.6:-0.1:-6.3;
disp3_list = 0.1:0.2:1.7;


disp2_list = [-5.5 -5.4 -5.3 -5.2]


% bandpass = 0.3
% disp2_list = [-4.8:-0.1:-5.4];
% disp3_list = [-0.5:0.2:0.5];

% bandpass = 0.45
% disp2_list = -5.:-0.1:-5.8;
% disp3_list = [-0.3:0.2:0.7];
...
for bp = bandpass
    ...
    ws.plot_status
    ws.write2WS
    filedir = strcat(filedir_glob,'\',sprintf('bp_%.3f_THz', bp),'\');
    %%
    for ii = 1:length(disp3_list)
        for jj = 1:length(disp2_list)
            close all
            disp2 = disp2_list(jj);
            disp3 = disp3_list(ii);
...
%%
function [electrical_pulse_width,electrical_pulse_width_error] = get_autocorr_FWHM_multipeak(filename)
