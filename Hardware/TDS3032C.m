classdef TDS3032C < handle
    ...
    methods
        function obj = TDS3032C(visaAddress)
            if nargin > 0
                obj.visaResourceString = visaAddress;
            else
                obj.visaResourceString = 'GPIB0::9::INSTR';
            end
        ...
        function connect(Obj)
            ...
            fopen(Obj.visaObj);
            disp('TDS3032C OSCilloscope connected');
        end
        ...
        function disconnect(Obj)
            ...
            if (Obj.isconnected)
                disp('TDS3032C OSCilloscope disconnected');
            end
            fclose(Obj.visaObj);
        end
    ...
    methods
        function wvfm = ReadTrace(obj, trace)
            % trace = 1|2|3|4
            obj.Write(strcat("DAT:SOURCE CH", num2str(trace))); % select trace
            % obj.Write('DAT:ENC RIBinary');
            % obj.Write('WFMPre:BN_Fmt RI')
            obj.Write('DAT:ENC ASCI');
%             obj.Write('WFMPre:PT_Fmt Y')
            obj.Write('DAT:WID 1'); % bytes per point, nbits = 8*nbytes
