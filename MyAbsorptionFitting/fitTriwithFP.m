...
%             figure
            ...
            filename_display = data_filename(tt(end)+1:end-4);
            title(sprintf("%s\nFP 2nd fitting result, %g nm\n T=%g. r1r1=%g",filename_display,lambda,fp_fit.T, ( 1-sqrt(1-fp_fit.B^2) )/fp_fit.B ),'Interpreter', 'none');
...
%     alpha_est = 10000; % SiN est.
    kappa = kappae+kappa0;
    ita = kappae/kappa;
    alpha_est = (linewidth_est - kappa/2 )/(2*ita/kappa)/(fp_fit.A0);
%     alpha_est = -20000;
...
%     findmin_fit_result = [findmin_fit_result(1:4), kappa0, kappae, findmin_fit_result(5:6)]; 
 ...
 % --Default!!! Only Alpha and x0
    ...
    triangle_fit_result = modtrans(fp_fit.A0*(1+r1r2^2),r1r2,fp_fit.x1,fp_fit.T,kappa0,kappae,...
     ...
     findmin_fit_result = [fp_fit.A0*(1+r1r2^2), r1r2, fp_fit.x1, fp_fit.T, kappa0, kappae, findmin_fit_result(1), findmin_fit_result(2)]; 
%      



 % --Fit Alpha and x0, and Q
%     findmin_fun = @(paras)LCL(modtrans_residual(fp_fit.A0*(1+r1r2^2),r1r2,fp_fit.x1,fp_fit.T,paras(3),paras(4),paras(1),paras(2),x_freq.',Trans_raw) ,dip_fit_weight);
% %     findmin_start_point = [mid_x(1)-0.5*linewidth_est, 0.5*alpha_est];
%     findmin_start_point = [x_freq(mid_x(1)), alpha_est, kappa0, kappae];
%     findmin_fit_result = fminsearch(findmin_fun,findmin_start_point);
%     triangle_fit_result = modtrans(fp_fit.A0*(1+r1r2^2),r1r2,fp_fit.x1,fp_fit.T,findmin_fit_result(3),findmin_fit_result(4),...
%                                findmin_fit_result(1),findmin_fit_result(2),x_freq.');
%      findmin_fit_result = [fp_fit.A0*(1+r1r2^2), r1r2, fp_fit.x1, fp_fit.T, findmin_fit_result(3),findmin_fit_result(4), findmin_fit_result(1),findmin_fit_result(2)]; 
...
%                                    mid_x(1), 0.5*alpha_est,(1:length(Trans_raw)).');
    ...
    fitted_Q0 = 299792.458/lambda/( findmin_fit_result(5) /MZI_fit_T * MZI_FSR );
    fitted_Qe = 299792.458/lambda/( findmin_fit_result(6) /MZI_fit_T * MZI_FSR );
