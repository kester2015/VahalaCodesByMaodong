...
%         filedirGlob = strcat('Z:\Qifan\SiN\20201002-thermal-rawdata\No14\',num2str(wavelength),'nm-02-mat');
    ...
    filedirGlob = 'Z:\Qifan\thin SiN\20201014-thermal-rawdata-checkspeed\1560.15nm-vpp-1.1-off--0.2-mat';
...
%     [mode_Q0, mode_Qe,~,~] = getQwithFP(Q_data_filename);
    ...
    for jj = 1:length(freqList)
        for ii = 1:length(powerList)
            pp = powerList(ii);
            close all
            this_filename = strcat(filedirGlob,'\Sweep_',num2str(freqList(jj)),'Hz_Power_',num2str(pp),'V.mat');
            temp = fitTriwithFP(this_filename,mode_Q0, mode_Qe,lambda,1);
            fitting_results(ii,2:end) = temp;
            fprintf('voltage: %g ',pp);
        end
        save(strcat(filedirGlob,'\Fitting_results\coefficients_',num2str(freqList(jj)),'Hz.mat'),'fitting_results');
    end

    figure;
    for jj = 1:length(freqList)
        load(strcat(filedirGlob,'\Fitting_results\coefficients_',num2str(freqList(jj)),'Hz.mat'),'fitting_results');
        scatter(jj, fitting_results(:,end),'DisplayName',strcat(num2str(freqList(jj)),'Hz') );
        hold on
    end
    legend;
...
%     for scan_round = [1 2]
%     filedirGlob = strcat('Z:\Qifan\SiN\20201004-thermal-rawdata\No14\',num2str(wavelength),'nm-0',num2str(scan_round),'-mat');
    load(strcat(filedirGlob,'\Fitting_results\coefficients_',num2str(0.5),'Hz.mat'),'fitting_results');
