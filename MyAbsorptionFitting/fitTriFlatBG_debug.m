...
%%
lambda = 1560.1;
data_filename = 'Z:\Qifan\AlGaAs\20200915-thermal-rawdata\No5\1560.1nm-02-mat\Sweep_20Hz_Power_0.8V.mat' ;

Q_data_filename = 'Z:\Qifan\AlGaAs\20200915-thermal-rawdata\after-1560.1nm.mat';
load(Q_data_filename,'data_matrix');
Q_obj = Q_trace_fit(data_matrix(:,2),data_matrix(:,3),39.9553,1560.1, 0.4,'fanomzi');
mode_Q = Q_obj.get_Q;
mode_Q0 = mode_Q(1)
mode_Qe = mode_Q(2)

temp = fitTriFlatBG(data_filename,mode_Q0, mode_Qe,lambda)
%%
% % % %%
...
% % %     pos_fitrange = 4; % times of linewidth, extends over pos_dip. total range = dip range + fit range;
...
% % %     
% % %     bg_base = mean([Trans_raw(pos_fitstart:pos_fitstart); Trans_raw(pos_dipend:pos_fitend)]);
% % %     
...
% % %     findmin_start_point = [bg_base, kappa0,kappae, mid_x(1), 0.1*alpha_est];
...
% % % %     findmin_start_point = [bg_base, kappae, mid_x(1), 0.05*alpha_est];
% % % %     findmin_fit_result = fminsearch(findmin_fun,findmin_start_point,options);
% % % %     triangle_fit_result = modtrans(findmin_fit_result(1),...
