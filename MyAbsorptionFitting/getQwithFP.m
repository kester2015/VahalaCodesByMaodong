...
%   Detailed explanation goes here
    ...
    fit_weight = ones(length(Q_trace_tofit),1);
    weight_start = max(pos_peak-2*linewidth_estimate,1);
    weight_end   = min(pos_peak+2*linewidth_estimate,length(Q_trace_tofit));
    fit_weight(weight_start:weight_end) = max(round(length(Q_trace_tofit)/(weight_end-weight_start)),1);
    Q_withfp_fit=fit((1:length(Q_trace_tofit)).', Q_trace_tofit, fit_Lorentz_fp, ...
        'StartPoint', [fit_A0_estimate, fit_B_estimate, fit_x1_estimate, fit_T_estimate...
                    ...
                    'Weight',fit_weight); % Lorentz with FP fit, {'A0','B','x1','T','LP','LS','x0'}
