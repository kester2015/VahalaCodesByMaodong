...
%%%%%%%%%%%
tauR=1/17.48e9; % in s, round-trip time % for the ring is 224 GHz.
scale=1;
scanning=[
 10e-6, 160, 3.0;
 10e-6, 160, 10.0;
...
n2=1616e-20;  % nonlinear index for silica, unit m^2/W
...
Aeff_p=2.63e-13; %40e-12; % effective mode area for the primary soliton, unit m^2
Aeff_s=5e-13; % effective mode area for the Stokes soliton, unit m^2
Aeff_ps=7e-13; % effective mode area for crossing term
...
D2p=2*pi*54.4e3;                        % Dispersion coefficient in the unit like ps/nm/km
D1p=2*pi*FSR;
...
nt=4096;        % set the point number to be 2048
...
%%
%Atmp=Atmp0;
PComb=0.13*1e-3; % 3 mW per comb line
NComb=12;   % half number of the comb line
...
for Kloop=1:1
    for sm=sn:sn  
        ...
        %fig_num=2;
        for kz=1:nz*5   
            delta0=(10*kz/nz)*(alpharing+kappa)/2;
%             delta0 = 5*(alpharing+kappa)/2;
...
spect=10*log10(abs(fftshift(fft(Asave1(:,end/10)))).^2);
wave=2*pi*c./fftshift((w+w0))*1e9;
figure
plot(wave,spect);
...
hold on
plot(time*1e12,abs(Ein_pulse).^2);
plot(time*1e12,abs( Asave1(:,(end/10)) ).^2)
ylim([0 1.1*max( abs(Ein_pulse).^2 )])
xlim([23 33])
